	...
	while lijstSchepenComputer or lijstSchepenGebruiker != []:
		...
		def shotComputer(self):
			...
			if self.randomShot in self.lijstSchepenGebruiker:
				...
				self.grid2.addWidget(QtGui.QLabel("o"), coordrij+10, coordkolom+10)
				# kleur rood
...
class SchepenKiezen(self):
	...
	def eindschip(self, lengte):
		self.lengte = lengte
		self.eindcoord1 = QtGui.QInputDialog.getText(self, 'Schiet Coordinaat', 'Vul hier het eindcoordinaat wat mag' + self.lengte + 'coordinaten van het begincoordinaat afligt in.')
